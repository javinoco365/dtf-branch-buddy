...
function AjustesPage() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["textil-marcas"] });
  ...
  const setDef = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["empresa-global"] });
      toast.success("Marca por defecto actualizada");
    },
  });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Gestiona las marcas comerciales que puedes usar en presupuestos y facturas. Los datos
          fiscales (SL) siguen siendo únicos.
        </p>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                Se sugerirá al crear nuevos presupuestos/facturas.
              </div>
            </div>
            <div className="w-64">
              <Select
                value={defaultId ?? "__none__"}
                onValueChange={(v) => setDef.mutate(v === "__none__" ? null : v)}
              >
                <SelectTrigger>
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        {marcas.length === 0 && (
          <div className="text-muted-foreground text-sm">No hay marcas creadas.</div>
        )}
        {marcas.map((m: any) => (
          <Card key={m.id} className={defaultId === m.id ? "border-primary" : ""}>
            <CardContent className="p-4 space-y-2">
              ...
              {(m.email || m.telefono) && (
                <div className="text-xs text-muted-foreground">
                  {[m.email, m.telefono].filter(Boolean).join(" · ")}
                </div>
              )}
            </CardContent>
          </Card>
...
function MarcaDialog({ open, onOpenChange, marca, onSave, loading }: any) {
  ...
  return (
    <Dialog
      ...
      onOpenChange={(o) => {
        onOpenChange(o);
        if (o) setF(marca ?? { nombre: "", color: "#3b82f6", activa: true });
      }}
    >
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{marca ? "Editar marca" : "Nueva marca comercial"}</DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 flex items-center gap-2">
            <Switch
              ...
              onCheckedChange={(v) => setF({ ...f, activa: v })}
            />
            <Label>Activa</Label>
          </div>
        </div>
