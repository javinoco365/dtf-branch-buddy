...
function FacturasPage() {
  ...
  const del = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["textil-facturas"] }); toast.success("Eliminada"); },
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Las facturas se generan al convertir un presupuesto aceptado.</p>
      </div>
      <Card>
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {data.length === 0 && <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">Sin facturas.</TableCell></TableRow>}
              {data.map((f: any) => (
                <TableRow key={f.id}>
                  ...
                  <TableCell>
                    {f.marca ? (
                      <span className="flex items-center gap-1.5 text-xs">
                        <span className="h-2 w-2 rounded-full" style={{ background: f.marca.color ?? "#3b82f6" }} />
                        {f.marca.nombre}
                      </span>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="icon" onClick={() => { if (confirm("¿Eliminar?")) del.mutate(f.id); }}>
